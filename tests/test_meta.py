...
def minimal_meta_v2():
    return {
        'schema_version': 2,
        'memory_dependency_hash': 'a' * 64,
        'used_memory_ids': {
            'terms': [], 'facts': [], 'claims': [],
            'style_rules': [], 'resolutions': [],
        },
        'new_entities': [],
        'alias_hypotheses': [],
        'attribute_hypotheses': [],
...
class ValidateMetaTests(unittest.TestCase):
    ...
    def test_accepts_v2_knowledge_observations(self):
        data = minimal_meta_v2()
        evidence = {'segment_id': 'seg-1234', 'quote': 'The claim is conditional.'}
        data['new_terms'] = [{
            'surface': 'consideration',
            'sense': 'contract-law-value-exchanged',
            'target_proposal': '对价',
            'category': 'legal-term',
            'domain': 'legal',
            'usage_note': '不得译为“考虑”',
            'forbidden_variants': ['考虑'],
            'evidence': evidence,
        }]
        data['new_claims'] = [{
            'holder': 'author',
            'proposition': 'The obligation applies only if consideration exists.',
            'polarity': 'affirmed',
            'modality': 'conditional',
            'scope': 'chapter',
            'target_gloss': '义务的成立以存在对价为条件。',
            'evidence': evidence,
        }]
        meta.validate_meta(data)

    def test_v2_requires_dependency_hash(self):
        data = minimal_meta_v2()
        data['memory_dependency_hash'] = 'not-a-hash'
        with self.assertRaises(ValueError) as ctx:
            meta.validate_meta(data)
        self.assertIn('memory_dependency_hash', str(ctx.exception))

    def test_v2_requires_structured_evidence(self):
        data = minimal_meta_v2()
        data['new_claims'] = [{
            'holder': 'author', 'proposition': 'P', 'polarity': 'affirmed',
            'modality': 'certain', 'scope': 'book', 'evidence': 'not structured',
        }]
        with self.assertRaises(ValueError) as ctx:
            meta.validate_meta(data)
        self.assertIn('evidence', str(ctx.exception))

    def test_v2_accepts_bounded_segment_translation_memory(self):
        data = minimal_meta_v2()
        data['segment_translations'] = [
            {'segment_id': 'seg-1234', 'target_text': '经审定的段落译文。'}
        ]
        meta.validate_meta(data)

    def test_load_rejects_oversized_or_symlink_sidecar(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, 'output_chunk0001.meta.json')
            with open(path, 'wb') as f:
                f.truncate(meta.MAX_SIDECAR_BYTES + 1)
            ...
            link = os.path.join(tmp, 'output_chunk0002.meta.json')
            try:
