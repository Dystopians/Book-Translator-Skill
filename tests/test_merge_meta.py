...
class DecisionsFileCliTests(unittest.TestCase):
    def test_main_reads_decisions_file_without_shell_pipe(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_apply(temp_dir, decisions_stream=None):
                captured["temp_dir"] = temp_dir
                captured["doc"] = json.load(decisions_stream)
            ...
            with mock.patch.object(
                ...
                merge_meta, "cmd_apply_merge", side_effect=fake_apply
            ):
                merge_meta.main()

        self.assertEqual(captured["temp_dir"], tmp)
        self.assertEqual(captured["doc"]["decisions"], [])
