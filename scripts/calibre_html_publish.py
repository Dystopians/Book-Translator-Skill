...
def extract_html_metadata(html_file):
    ...
    try:
        ...
        if author_match:
            author = _html_lib.unescape(author_match.group(1)).strip()
        else:
...
def main():
    ...
    try:
        # Create a unique working directory in the same directory as the input
        # so relative image paths remain local. A fixed
        # `<base>_conversion_temp` directory could collide with user data and
        # was recursively deleted after a successful conversion.
        ...
        conversion_workspace = tempfile.TemporaryDirectory(
            prefix=f"{base_name}_conversion_",
            dir=input_dir,
        )
        temp_dir = conversion_workspace.name
