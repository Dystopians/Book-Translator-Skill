...
DRAFT_ARTIFACTS = {
    'output.md': 'output.draft.md',
    'output.html': 'output.draft.html',
    'book.html': 'book.draft.html',
    'book_doc.html': 'book_doc.draft.html',
    'book.docx': 'book.draft.docx',
    'book.epub': 'book.draft.epub',
    'book.pdf': 'book.draft.pdf',
}
...
# Drop script/style payloads completely. Other non-allowlisted containers lose
# their tags and attributes but keep recursively sanitized visible fallback
# text, avoiding needless loss of book content.
_DROP_CONTENT_TAGS = frozenset({'script', 'style'})
_DROP_EMPTY_TAGS = frozenset({'base', 'embed', 'input', 'link', 'meta', 'source', 'track'})
_GLOBAL_SAFE_ATTRS = frozenset({
    'aria-hidden', 'aria-label', 'class', 'dir', 'id', 'lang', 'role', 'title',
})
_TAG_SAFE_ATTRS = {
...
_URL_ATTRS = frozenset({'cite', 'href', 'src'})
...
def _safe_html_url(tag, attr_name, value):
    """Return whether an HTML URL attribute is safe to keep."""
    decoded = _html_lib.unescape(value).strip()
    compact = re.sub(r'[\s\x00-\x20\x7f]+', '', decoded).lower()
    scheme_match = re.match(r'^([a-z][a-z0-9+.\-]*):', compact)
    if not scheme_match:
        return True
    ...
    if scheme == 'data':
        return (
            tag == 'img'
            and attr_name == 'src'
            and bool(_SAFE_IMAGE_DATA_URL_RE.match(compact))
        )
    return scheme in _SAFE_LINK_SCHEMES


class _SafeHtmlFragmentParser(HTMLParser):
    """Rebuild a book-body fragment from a conservative HTML allowlist."""
    ...
    def _safe_attrs(self, tag, attrs):
        allowed = _GLOBAL_SAFE_ATTRS | _TAG_SAFE_ATTRS.get(tag, frozenset())
        rendered = []
        seen = set()
        for raw_name, value in attrs:
            name = (raw_name or '').lower()
            if name in seen or name not in allowed or name.startswith('on'):
            ...
            if value is None:
                ...
                continue
            if name in _URL_ATTRS and not _safe_html_url(tag, name, value):
                self.removed_items += 1
                continue
            rendered.append(
                f' {name}="{_html_lib.escape(value, quote=True)}"'
            )
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if self.blocked_tags:
            if tag not in _VOID_HTML_TAGS and tag not in _DROP_EMPTY_TAGS:
        ...
        if tag in _DROP_CONTENT_TAGS:
            ...
            self.removed_items += 1
            return
        if tag in _DROP_EMPTY_TAGS or tag not in _SAFE_HTML_TAGS:
            self.removed_items += 1
            return
    ...
    def handle_startendtag(self, tag, attrs):
        ...
        if self.blocked_tags:
            return
        if tag in _DROP_CONTENT_TAGS or tag in _DROP_EMPTY_TAGS:
            self.removed_items += 1
            return
    ...
    def handle_endtag(self, tag):
        ...
        if self.blocked_tags:
            if tag in self.blocked_tags:
                match_index = len(self.blocked_tags) - 1 - self.blocked_tags[::-1].index(tag)
                del self.blocked_tags[match_index:]
            return
    ...
    def handle_data(self, data):
        if not self.blocked_tags:
            self.parts.append(_html_lib.escape(data, quote=False))
...
def sanitize_html_fragment(fragment):
    """Remove active HTML while preserving common book formatting."""
    parser = _SafeHtmlFragmentParser()
    parser.feed(fragment)
    parser.close()
    return ''.join(parser.parts), parser.removed_items
...
def convert_with_python_markdown(md_file, html_file, title):
    ...
    try:
        ...
        safe_title = _html_lib.escape(title, quote=False)
...
def convert_with_basic_regex(md_file, html_file, title):
    ...
    try:
        ...
        # Links — quote attribute values before the final HTML sanitizer runs.
        def _md_link_to_html(m):
            label = m.group(1)
            href = _html_lib.escape(m.group(2), quote=True)
            return f'<a href="{href}">{label}</a>'
        html_content = re.sub(r'\[([^\]]*)\]\(([^)]*)\)', _md_link_to_html, html_content)
...
def apply_template_to_html(html_content, template_file, output_file, title, lang_cfg, author=None):
    ...
    try:
        ...
        def fill_metadata(shell):
            replacements = {
                '$title$': _html_lib.escape(title, quote=False),
                '$lang$': _html_lib.escape(lang_cfg['lang_attr'], quote=True),
                '$body_font$': lang_cfg['font_family'],
                '$toc_label$': lang_cfg['toc_label'],
            }
            shell = re.sub(
                r'\$(?:title|lang|body_font|toc_label)\$',
                lambda match: replacements[match.group(0)],
                shell,
            )
            if author and ('<head>' in shell or '<head ' in shell):
                author_meta = (
                    f'<meta name="author" '
                    f'content="{_html_lib.escape(author, quote=True)}">'
                )
                shell = re.sub(
                    r'(<head[^>]*>)',
        ...
        # Fill metadata in the trusted template shell before inserting book
        # content. Otherwise literal `$title$`/`$lang$` text inside a book would
        # be mistaken for a template placeholder and silently corrupted.
        if '$body$' in template_content:
            before, after = template_content.split('$body$', 1)
            full_html = fill_metadata(before) + html_content + fill_metadata(after)
        elif '{{content}}' in template_content:
            before, after = template_content.split('{{content}}', 1)
            full_html = fill_metadata(before) + html_content + fill_metadata(after)
        else:
            shell = fill_metadata(template_content)
            if '</body>' in shell:
                full_html = shell.replace(
                    '</body>',
                    f'{html_content}\n</body>',
                    1,
                )
            else:
                full_html = shell + html_content
...
def convert_md_to_html(temp_dir, title, lang_cfg, author=None):
    ...
    # Match through the final </body>. A source book may contain raw `</body>`
    # text; a non-greedy match would let that truncate the rest of the book
    # before the sanitizer sees it.
    body_match = re.search(r'<body[^>]*>(.*)</body>', html_content, re.DOTALL | re.IGNORECASE)
    body_content = body_match.group(1).strip() if body_match else html_content
    body_content, removed_items = sanitize_html_fragment(body_content)
    if removed_items:
        print(f"Sanitized {removed_items} unsafe HTML element(s) or attribute(s)")
...
def generate_simple_toc_html(toc_data):
    ...
    for item in toc_data:
        level = item['level']
        text = _html_lib.escape(str(item['text']), quote=False)
        heading_id = _html_lib.escape(str(item['id']), quote=True)
...
def insert_toc_with_regex(html_file):
    ...
    if re.search(toc_content_pattern, html_content, re.DOTALL):
        html_content = re.sub(
            toc_content_pattern,
            lambda match: match.group(1) + toc_html + match.group(2),
            html_content,
            flags=re.DOTALL
        )
...
def _validate_export_name(name):
    ...
    if len(stem) > 180:
        raise ValueError("--export-name must be 180 characters or fewer")
    if stem in {'.', '..'} or stem.endswith('.'):
        raise ValueError("--export-name is not a portable filename stem")
    if any(ord(ch) < 32 for ch in stem) or re.search(r'[<>:"|?*]', stem):
        raise ValueError("--export-name contains characters invalid in portable filenames")

    windows_base = stem.split('.', 1)[0].rstrip(' .').upper()
    windows_reserved = {
        'CON', 'PRN', 'AUX', 'NUL',
        *(f'COM{i}' for i in range(1, 10)),
        *(f'LPT{i}' for i in range(1, 10)),
    }
    if windows_base in windows_reserved:
        raise ValueError("--export-name is a reserved Windows filename")
    if stem.casefold() in {'book', 'book_doc'}:
        raise ValueError(
            f"--export-name {stem!r} is reserved for canonical pipeline artifacts"
        )
    return stem
...
def export_named_aliases(temp_dir, export_name):
    ...
    canonical_paths = {
        os.path.normcase(os.path.abspath(os.path.join(temp_dir, name))): name
    ...
    for src_name, dst_name in mappings.items():
        ...
        owner = canonical_paths.get(dst_key)
        if owner is not None and owner != src_name:
            raise ValueError(
                f"--export-name would overwrite canonical artifact {owner!r}"
            )
        planned.append((src_name, dst_name, src, dst, src_key, dst_key))

    copied = []
    for src_name, dst_name, src, dst, src_key, dst_key in planned:
...
def _run_quality_gate(temp_dir, mode):
    """Evaluate the evidence/review gate without trusting cached CLI output."""
    try:
        from quality_gate import evaluate_gate
    except ImportError as exc:
        raise RuntimeError(
            "quality_gate.py is required for --quality-mode draft/final"
        ) from exc
    result = evaluate_gate(temp_dir, mode=mode, ingest_reviews=True)
    if not isinstance(result, dict) or 'ready_for_final' not in result:
        raise RuntimeError("quality gate returned an invalid result")
    return result


def _enforce_quality_mode_state(temp_dir, quality_mode):
    """An enhanced task may never fall back to the ungated legacy publisher."""
    database = os.path.join(temp_dir, 'translation_state.sqlite3')
    if quality_mode == 'legacy' and os.path.lexists(database):
        raise ValueError(
            "translation_state.sqlite3 exists; choose --quality-mode draft or final "
            "instead of bypassing the evidence/review gate"
        )


def _safe_copy_asset_tree(source, destination):
    """Copy an asset tree while refusing symlinks and special files."""
    ...
    for current, dirnames, filenames in os.walk(source, followlinks=False):
        for name in list(dirnames):
            ...
            if os.path.islink(candidate):
                raise ValueError(f"Draft assets may not contain symbolic links: {candidate}")
        ...
        for name in filenames:
            ...
            if os.path.islink(src) or not os.path.isfile(src):
                raise ValueError(f"Draft asset must be a regular non-link file: {src}")
            shutil.copy2(src, os.path.join(target_dir, name))


def _prepare_draft_workspace(temp_dir):
    """Create an isolated build tree so drafts never overwrite final names."""
    holder = tempfile.TemporaryDirectory(prefix='.translate-book-draft-', dir=temp_dir)
    workspace = holder.name
    names = {'manifest.json', 'config.txt', 'source_fingerprint.json'}
    ...
    try:
        for name in sorted(names):
            ...
            if os.path.islink(source) or not os.path.isfile(source):
                raise ValueError(f"Draft input must be a regular non-link file: {source}")
            shutil.copy2(source, os.path.join(workspace, name))
        images = os.path.join(temp_dir, 'images')
        if os.path.isdir(images):
            if os.path.islink(images):
                raise ValueError(f"Draft image directory may not be a symbolic link: {images}")
            _safe_copy_asset_tree(images, os.path.join(workspace, 'images'))
    except Exception:
        holder.cleanup()
        raise
...
def _publish_draft_artifacts(build_dir, temp_dir, export_name=None):
    """Copy isolated build results to names that cannot masquerade as final."""
    copied = []
    for source_name, draft_name in DRAFT_ARTIFACTS.items():
        source = os.path.join(build_dir, source_name)
        if not os.path.isfile(source) or os.path.islink(source):
            continue
        destination = os.path.join(temp_dir, draft_name)
        shutil.copy2(source, destination)
        copied.append(draft_name)

    if export_name:
        stem = _validate_export_name(export_name)
        for canonical, suffix in (
            ('book.draft.html', '.draft.html'),
            ('book.draft.docx', '.draft.docx'),
            ('book.draft.epub', '.draft.epub'),
            ('book.draft.pdf', '.draft.pdf'),
        ):
            source = os.path.join(temp_dir, canonical)
            if os.path.isfile(source):
                alias = f"{stem}{suffix}"
                shutil.copy2(source, os.path.join(temp_dir, alias))
                copied.append(alias)
    return copied
...
def main():
    ...
    parser.add_argument(
        '--quality-mode', choices=('legacy', 'draft', 'final'), default='legacy',
        help='Evidence/review gate mode; draft uses non-final artifact names',
    )
    parser.add_argument(
        '--cleanup', action='store_true',
        help='Remove rebuildable conversion artifacts but preserve resumable translation state',
    )
    parser.add_argument(
        '--cleanup-level', choices=('transient', 'aggressive'), default='transient',
        help='Aggressive cleanup also deletes resumable state and requires confirmation',
    )
    parser.add_argument(
        '--confirm-delete-state', default=None, metavar='TOKEN',
        help='Required literal DELETE_TRANSLATION_STATE for aggressive cleanup',
    )
    ...
    if args.cleanup_level == 'aggressive':
        if not args.cleanup or args.confirm_delete_state != 'DELETE_TRANSLATION_STATE':
            print(
                "Error: aggressive cleanup requires --cleanup and "
                "--confirm-delete-state DELETE_TRANSLATION_STATE"
            )
            sys.exit(1)

    try:
        _enforce_quality_mode_state(temp_dir, args.quality_mode)
    except ValueError as exc:
        print(f"Error: {exc}")
        sys.exit(1)

    if args.quality_mode != 'legacy':
        try:
            gate_result = _run_quality_gate(temp_dir, args.quality_mode)
        except Exception as exc:
            print(f"Error: quality gate failed: {exc}")
            sys.exit(1)
        if args.quality_mode == 'final' and not gate_result.get('ready_for_final'):
            blockers = gate_result.get('blockers', [])
            print(f"Error: final publication blocked by {len(blockers)} quality issue(s)")
    ...
    draft_holder = None
    build_dir = temp_dir
    if args.quality_mode == 'draft':
        try:
            draft_holder, build_dir = _prepare_draft_workspace(temp_dir)
        except (OSError, ValueError) as exc:
            print(f"Error: could not prepare isolated draft build: {exc}")
            sys.exit(1)

    # Step 4: Merge
    if not merge_markdown_files(build_dir):
        if draft_holder:
            draft_holder.cleanup()
        sys.exit(1)

    # Step 5: Convert to HTML
    if not convert_md_to_html(build_dir, title, lang_cfg, author):
        if draft_holder:
            draft_holder.cleanup()
        sys.exit(1)
    ...
    all_formats_ok = generate_formats(build_dir, lang_cfg['lang_attr'], cover=cover)

    draft_artifacts = []
    if args.quality_mode == 'draft':
        try:
            draft_artifacts = _publish_draft_artifacts(build_dir, temp_dir, export_name)
        finally:
            draft_holder.cleanup()
            draft_holder = None
    elif export_name:
        if all_formats_ok:
            aliases = export_named_aliases(build_dir, export_name)
            if aliases:
                print("\nExport aliases:")
                for name in aliases:
                    print(f"  {name}")
    ...
    output_names = (
        draft_artifacts
        if args.quality_mode == 'draft'
        else ['book.html', 'book_doc.html', 'book.docx', 'book.epub', 'book.pdf']
    )
    for ext in output_names:
    ...
    if args.cleanup:
        if all_formats_ok:
            cleanup_intermediate_files(
                temp_dir,
                aggressive=(args.cleanup_level == 'aggressive'),
            )
        else:
            print("\nSkipping cleanup — some formats failed. Intermediate files kept for diagnosis/retry.")


def cleanup_intermediate_files(temp_dir, aggressive=False):
    """Remove rebuildable artifacts; preserve resumable state by default."""
    ...
    names = ['input.html', 'input.md', 'output.html']
    if aggressive:
        names.extend([
            'translation_state.sqlite3',
            'translation_state.sqlite3-journal',
            'translation_state.sqlite3-wal',
            'translation_state.sqlite3-shm',
            'run_state.json',
            'glossary.json',
        ])
    for name in names:
    ...
    if removed:
        ...
        if chunk_files:
            print(f"  {len(chunk_files)} chunk/sidecar files")
