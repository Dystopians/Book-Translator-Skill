...
- `apply-merge <temp_dir>` — read a decisions JSON from stdin or
  `--decisions-file`, conservatively apply the auto-applies and resolved
  decisions to the glossary, and write meta content hashes into
  `applied_meta_hashes` for every chunk in `consumed_chunk_ids`. Atomic save.
...
MAX_DECISIONS_BYTES = 64 * 1024 * 1024
EVIDENCE_REFS_CAP = 5
...
def cmd_apply_merge(temp_dir, decisions_stream=None):
    """Read decisions JSON and apply it to the glossary."""
    stream = decisions_stream if decisions_stream is not None else sys.stdin
    source_label = getattr(stream, 'name', None) or "stdin"
    try:
        decisions_doc = json.load(stream)
    except json.JSONDecodeError as e:
        sys.stderr.write(
            f"error: decisions JSON from {source_label} is not valid: {e}\n"
        )
        sys.exit(2)
...
def main():
    ...
    p_app = sub.add_parser(
        'apply-merge',
        help="Apply merge decisions from stdin or a JSON file",
    )
    p_app.add_argument('temp_dir')
    p_app.add_argument(
        '--decisions-file',
        default=None,
        help="Read decisions JSON from this file instead of stdin",
    )
    ...
    if args.cmd == 'prepare-merge':
        ...
        if args.decisions_file:
            try:
                if os.path.islink(args.decisions_file):
                    raise OSError("symbolic-link decisions files are not allowed")
                if os.path.getsize(args.decisions_file) > MAX_DECISIONS_BYTES:
                    raise OSError(
                        f"decisions file exceeds {MAX_DECISIONS_BYTES} bytes"
                    )
                with open(args.decisions_file, 'r', encoding='utf-8') as stream:
                    cmd_apply_merge(args.temp_dir, decisions_stream=stream)
            except OSError as e:
                print(f"error: cannot read decisions file: {e}", file=sys.stderr)
                sys.exit(2)
        else:
